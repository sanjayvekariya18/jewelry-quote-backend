...
export default class BaseHandler {
    ...
    /* start modify validation rules functions */
        ...
        private callbackRulesInitiazer() {
            ...
            let isCallbackRules: any = typeof this.validationFormRules?.callback == "function" 
                && !helper.isEmpty(this.validationFormRules?.callback(this.requestData));
    ...
    /* start failed validation errors */
            ...
            for (let error of Object.entries(this.failedValidationErrors)) {
                ...
                _errors[field] = errorMsgs.map((messages: any) => {
                    ...
                    if (messages.includes(":mime_types")) {
                        ...
                        messages = messages.replace(":mime_types", attributeMimeTypes)
                            .replace(':file_name', fileObj.name || "");
                    }
                    ...
                    if (messages.includes(":max_size")) {
                        ...
                        messages = messages.replace(":max_size", helper.formatBytes(attributeValue))
                            .replace(':file_name', fileObj.name || "");
                    }
                    ...
                    if (messages.includes(":min_size")) {
                        ...
                        messages = messages.replace(":min_size", helper.formatBytes(attributeValue))
                            .replace(':file_name', fileObj.name || "");
                    }
        ...
        */
        private getCustomeValidationMessages() {
            ...
            const customValidationMessages: any = Object.entries(validationMessages)
                .map(([rule, message]) => ({
                    rule, 
                    message
                }));
            ...
            Object.entries(this.failedValidationErrors).forEach(([key, _errors]: any) => {
                this.failedValidationErrors[key] = _errors.map((error: any) => {
                    ...
                    if (_validationObj) {
                        ...
                        return error.replace(/  +/g, ' ').split(' ').map((_item: any) => {
                            if (_item.includes(':_')) {
